...
class PppoeHost {
    ...
    method start {} {
        set tag "body PppoeHost::start [info script]"
    Deputs "----- TAG: $tag -----"
        ixNet exec pppoxStart $handle
            
        return [GetStandardReturnHeader]
    }
    method stop {} {
        set tag "body PppoeHost::stop [info script]"
    Deputs "----- TAG: $tag -----"
        ixNet exec pppoxStop $handle
...
body PppoeHost::get_fh_stats { args } {

    set tag "body PppoeHost::get_fh_stats [info script]"
Deputs "----- TAG: $tag -----"
    set sessions 1
    set sessionsup 1
    set succsetuprate 1
    foreach { key value } $args {
        ...
        switch -exact -- $key {
            ...
            -sessionsup {
				set sessionsup $value
			}
            -succsetuprate {
				set succsetuprate $value
			}
		}
    }   
